...
const accessoriesData = [
 ...
 // Cart Data
        ...
        // Shop Icon - Toggle Categories Dropdown
        ...
        function updateActiveFilter() {
            const filterBtns = productFilter.querySelectorAll('.filter-btn');
            filterBtns.forEach(btn => {
                btn.classList.remove('active');
            });
            
            const activeBtn = Array.from(filterBtns).find(btn => 
                btn.textContent.toLowerCase().replace(/\s+/g, '') === currentCategory.replace('allproducts', 'all')
            );
            if (activeBtn) {
        ...
        function loadProducts() {
            ...
            const filteredProducts = currentCategory === 'allproducts' 
                ? products 
        ...
        function updateCategoryCounts() {
            ...
            categories.forEach(category => {
                const count = category === 'allproducts' 
                    ? products.length 
        ...
        function selectProduct(productId) {
            ...
            if (product) {
                ...
                // Scroll to product
                setTimeout(() => {
                    const productCard = document.querySelector(`.product-card:nth-child(${products.indexOf(product) + 1})`);
                    if (productCard) {
        ...
        function switchToLogin() {
            ...
            authForm.innerHTML = `
                ...
                <div class="auth-switch" onclick="switchToSignUp()">
            ...
            cart.forEach(item => {
                ...
                cartItem.innerHTML = `
                    ...
                    <div class="cart-item-details">
                        ...
                        <div style="margin-top: 5px; font-weight: bold;">$${itemTotal.toFixed(2)}</div>
                    </div>
                `;
        ...
        document.addEventListener('click', function(event) {
            ...
            const isClickOnShopButton = event.target.closest('#shop-button');
            const isClickInsideSearch = searchBar.contains(event.target);
            const isClickOnSearchButton = event.target.closest('.button') && event.target.closest('.button').querySelector('svg[viewBox="0 0 24 24"]');
            
            if (!isClickInsideDropdown && !isClickOnShopButton) {
                categoriesDropdown.classList.remove('show');
            }
